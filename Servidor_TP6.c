	...
	while ( (connfd = accept(fd,NULL, 0)) > 0 ){
		...
		if ( pid == 0 ) {
			...
			if(strcmp(lin,"user")==0){ //verificamos que el LINK este correcto // Añadir caso para archivo diferente a usuario.txt
				lin=strtok(NULL,"/");
				char * numeroarg=lin; //numero de la persona cuando se usa un argumento
				char * auxx;
				char * auxx2 = lin;
				lin=strtok(NULL,"/");
				auxx = strtok(lin,"");
				//printf("%s",auxx);
				linea=strtok(auxx," "); // posible argumento
				if(strcmp(linea,"email")==0 || strcmp(linea,"nombre")==0 || strcmp(linea,"tel")==0 || strcmp(linea,"ciudad")==0 || strcmp(linea,"cp")==0){
					...
					linea=auxx2;
					strtok(linea," "); // cuando no hay argumento linea vale el numero; cuando si lo hay, linea vale el argumento
					arg=0;
				}
				char palabra[50];
				if(arg==1)
				...
				char * puntero=strstr(buff2,palabra); // devuelve puntero al inicio de la palabra buscada
				if(puntero!=NULL){
					int argumento=0;
					if(arg==1){
						...
						if(strcmp(linea,"email")==0)
							argumento=1;
						if(strcmp(linea,"tel")==0)
							argumento=2;
						if(strcmp(linea,"ciudad")==0)
							argumento=3;
					...
					// El argumento me dice la cantidad de veces a buscar entre los delimitadores ':'
					...
					while(*puntero_aux!=':'){puntero_aux++;} // leer desde lo encontrado hasta el delimitador ':'
